...
fn test_parse_lrc() {
    ...
    let result = parsers::parse_lyrics(&content, LyricsRawTypes::Lrc);
...
fn test_parse_qrc() {
    ...
    let result = parsers::parse_lyrics(&content, LyricsRawTypes::Qrc);
...
fn test_parse_yrc() {
    ...
    let result = parsers::parse_lyrics(&content, LyricsRawTypes::Yrc);
...
fn test_parse_krc() {
    ...
    let result = parsers::parse_lyrics(&content, LyricsRawTypes::Krc);
...
fn test_parse_spotify() {
    ...
    let result = parsers::parse_lyrics(&content, LyricsRawTypes::Spotify);
...
fn test_parse_musixmatch() {
    ...
    let result = parsers::parse_lyrics(&content, LyricsRawTypes::Musixmatch);
...
fn test_parse_lyricify_syllable() {
    ...
    let result = parsers::parse_lyrics(&content, LyricsRawTypes::LyricifySyllable);
...
fn test_parse_lyricify_lines() {
    ...
    let result = parsers::parse_lyrics(&content, LyricsRawTypes::LyricifyLines);
...
fn test_parse_auto_detect_lrc() {
    let content = fs::read_to_string("tests/test_data/LrcDemo.txt").expect("Failed to read LRC file");
    let result = parsers::parse_lyrics_auto(&content);
...
fn test_generate_lrc() {
    let content = fs::read_to_string("tests/test_data/LrcDemo.txt").expect("Failed to read LRC file");
    let data = parsers::parse_lyrics(&content, LyricsRawTypes::Lrc).expect("Failed to parse LRC");

    let output = generators::generate_string(&data, LyricsTypes::Lrc);
...
fn test_generate_qrc() {
    let content = fs::read_to_string("tests/test_data/QrcDemo.txt").expect("Failed to read QRC file");
    let data = parsers::parse_lyrics(&content, LyricsRawTypes::Qrc).expect("Failed to parse QRC");

    let output = generators::generate_string(&data, LyricsTypes::Qrc);
...
fn test_generate_yrc() {
    let content = fs::read_to_string("tests/test_data/YrcDemo.txt").expect("Failed to read YRC file");
    let data = parsers::parse_lyrics(&content, LyricsRawTypes::Yrc).expect("Failed to parse YRC");

    let output = generators::generate_string(&data, LyricsTypes::Yrc);
...
fn test_roundtrip_lrc() {
    ...
    let generated = generators::generate_string(&data, LyricsTypes::Lrc).expect("Failed to generate LRC");
    let reparsed = parsers::parse_lyrics(&generated, LyricsRawTypes::Lrc).expect("Failed to re-parse LRC");
...
#[test]
fn test_string_helper_parse_time() {
    let ms = helpers::string_helper::get_milliseconds_from_string("02:05.500");
    assert_eq!(ms, Some(125500));
}
...
#[test]
fn test_string_helper_between() {
    let result = helpers::string_helper::between("[key:value]", "[", ":");
...
<tt xmlns="http://www.w3.org/ns/ttml" xmlns:itunes="http://music.apple.com/lyric-ttml-internal" xmlns:ttm="http://www.w3.org/ns/ttml#metadata" itunes:timing="Word" xml:lang="en">
  ...
  <body>
    ...
    let data = parsers::ttml_parser::parse(ttml);
...
fn test_parse_ttml_empty() {
    let data = parsers::ttml_parser::parse("");
