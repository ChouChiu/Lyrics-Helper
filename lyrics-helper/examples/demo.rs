...
fn cmd_decrypt_qrc(file_path: &str) {
    ...
    match lyrics_helper::decrypter::qrc::decrypter::decrypt_lyrics(&content) {
...
fn cmd_decrypt_krc(file_path: &str) {
    ...
    match lyrics_helper::decrypter::krc::decrypter::decrypt_lyrics(&content) {
        Some(decrypted) => println!("{}", decrypted),
...
fn cmd_generators_demo() {
    let path = "tests/test_data/LyricifySyllableDemo.txt";
    if !std::path::Path::new(path).exists() {
