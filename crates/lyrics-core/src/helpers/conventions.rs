...
/// 里的标签从显示文本与逐词时间里都去掉。只剩标签的行与本来就空着的行（网易云的行级歌词里
/// 常拿它当间隔）都不会留下，所以调用后的行数可能比调用前少。
///
...
///
/// 短语写在行尾时，它前面那个分隔符也不再留下：这一行的文字由它的词推导，两者必须一致。
/// 背景和声跟着这一行的分边（子行与主行由同一位唱）。
...
/// 不是随便哪一行的回答。已经带了背景和声的行不再收下一个回声（一行只带一个），回声并进来
/// 后跟着它所回答的那一行的分边。
...
///
/// 接通时：文字接成一句（音节行接的是词，接缝处补一个分隔符，因此一行的词拼起来仍正好等于
/// 这一行的文字）、两行各自的译文按语言接成一句、第一行没有背景和声时把后一行的收下、结束
/// 时间取两行较晚的那个。拼音不像译文那样接起来（下游也不接）：接起来的一行保留第一行原有
/// 的拼音。
